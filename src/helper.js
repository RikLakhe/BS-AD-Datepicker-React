  ...
  for (var i = 0; i < bsMonthData.length; i++) {
    ...
    if (totalYears <= yearCount) {
      if (
        ...
        return (
          ConfigBS.bsMonthUpperDays[bsMonth - 1][bsMonthUpperDaysIndex] - 2
        );
      } else {
  ...
  for (var i = 0; i < bsMonthData.length; i++) {
    ...
    if (yearDiff > yearCount + bsMonthData[i]) {
      ...
      monthDaysFromMinBsYear +=
        ConfigBS.bsMonthUpperDays[bsMonth - 1][bsMonthUpperDaysIndex] *
        bsMonthData[i];
    } else {
      monthDaysFromMinBsYear +=
        ConfigBS.bsMonthUpperDays[bsMonth - 1][bsMonthUpperDaysIndex] *
 ...
 */
export const refactoringDate = (value) => {
  ...
  if (value && !isEmpty(value)) {
    ...
    if (dashSplitValue.length === 3) {
      ...
      if (
        enterDate < 1 ||
        enterDate >
          parseInt(getDaysInMonthAD(enterYear, parseInt(enterMonth) + 1))
      ) {
        enterDate = 1;
      }
  ...
  if (calenderType === 'AD') {
    for (let i = 1; i <= getDaysInMonthAD(useYear, useMonth + 1); i++) {
      DynamicDatepickerCalender[
        getStartingDayAD(useYear, parseInt(useMonth) + 1) + i - 1
      ] = i;
    }
    ...
    let todayBS = covertADToBS(
      today.getFullYear(),
      today.getMonth(),
      today.getDate() - 1
    );

    let refactoringMinBS = refactoringDate(minDate);
    let minBS =
      refactoringMinBS &&
      covertADToBS(
        refactoringMinBS.year,
        refactoringMinBS.month,
        refactoringMinBS.date - 1
      );
    let minBSDate =
      minBS && `${minBS.bsYear}- ${minBS.bsMonth + 1}-${minBS.bsDate}`;
    let refactoringMaxBS = refactoringDate(maxDate);
    let maxBS =
      refactoringMaxBS &&
      covertADToBS(
        refactoringMaxBS.year,
        refactoringMaxBS.month,
        refactoringMaxBS.date
      );
    let maxBSDate =
      maxBS && `${maxBS.bsYear}- ${maxBS.bsMonth + 1}-${maxBS.bsDate}`;
      ...
      if (DynamicDatepickerCalender[i]) {
        domDataArray.push(
          <td
            className={`date-selectable 
              ${
                DynamicDatepickerCalender[i] === todayBS?.bsDate + 1 &&
                useMonth === todayBS?.bsMonth &&
 ...
 * @return Boolean
 */
const isClickAble = (year, month, date, minDate, maxDate) => {
  let minDateObject = refactoringDate(minDate);
  let maxDateObject = refactoringDate(maxDate);
